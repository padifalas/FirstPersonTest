...
public class FirstPersonControls : MonoBehaviour
{
    ...
    public void Jump()
    {
        ...
        {
            // ccalculate the jump velocity
            velocity.y = Mathf.Sqrt(jumpHeight * -2f * gravity);
        }
    }

    // this method/function will boosts player jump
    public void ApplyJumpBoost(float boostForce)
    {
        velocity.y = Mathf.Sqrt(boostForce * -2f * gravity);
    }
    
    private void OnControllerColliderHit(ControllerColliderHit hit) //this is for the skateboard
    {
        if (hit.collider.CompareTag("Skateboard"))
        {
